...
# Create function for basic download
basic_SWW_download <- function(country_code) {
  # 1. GDP table: 
...
# Write to XLXS in a function:
basic_SWW_Excel <- function(file_path,country_code){
  df_final <- basic_SWW_download(country_code)
  library(openxlsx)
  output_path <- gsub(file_path, pattern = "_Data_Scripts/", replacement = "_2014/")
  write.xlsx(df_final, file = paste0(output_path, "SWW_data.xlsx"), 
             colNames = TRUE, rowNames = FALSE, overwrite = TRUE)
}

basic_SWW_Excel(file_path = file_path, country_code = country_code)